...
function parseDataAndCreateTableOfExpense(data) {
    ...
    for (i = 0; i < data.length; i++) {
        ...
        monthlyExpenseToCategoryName.push({
            "categoryId": data[i].categoryName,
...
function drawTableOfExpense(dataExpense, dataOpacity, totalExpense, numberOfRows) {
    let tfoot = document.getElementById("total-analytics-table")
        .getElementsByTagName("tfoot")[0];
    clearTableOfExpense(tfoot);
    let tbody = document.getElementById("total-analytics-table")
...
function addRowBodyInTableOfExpense(totalExpense, numberOfRows) {
    let table = document.getElementById("total-analytics-table")
        .getElementsByTagName("tbody")[0];
...
$(document).on("click", "#total-analytics-table-foot td", function () {
    ...
    let category = $("#total-analytics-table-foot").find("tr").eq(rowIndex).find("td").eq(0).html();
    let month = $(this).closest("td").index() - 1;
    let year = $("#yearSelectAnnualAndMonthlyTotalStatistics").val();
    getTransactionsByCategoryAndMonth(year, month, category);
})

function getTransactionsByCategoryAndMonth(year, month, category) {
    $('#categoryInfoModal').modal('show');

    $.ajax({
        method: 'GET',
        url: '/transactions/info?year=' + year + '&month=' + month + '&category=' + category,
        contentType: 'application/json; charset=utf8',
        async: false,
        success: function (data) {
