    ...
    return {
        ...
        set: function (array) {
            if (array.length === 2) {
                ...
                console.log('incorrect length passed array to fibonacci.set(array)');
            }
        },
        reset: function() {
            fibonacciInner = [1, 1];
...
function getPeriodOfInactivity() {
    ...
    $.ajax({
        ...
        error: function () {
            console.log("Error receiving data for long polling");
        }
    })
...
function writeKeywordsOfCategory(category) {
    if (category.keywords.length === 0) {
        $('.keywords-list').append("No keywords")
    }
